...
import { formatCPF, formatPhone, validateCPF } from '@/lib/validations';
import { 
  Users, 
  LogOut, 
  Plus, 
...
const AdminDashboard = () => {
  ...
  const checkCPFExists = async (cpf: string, excludeId?: number) => {
    try {
      const cleanCPF = cpf.replace(/\D/g, '');
      let query = supabase
        .from('formulario_cepic')
        .select('id')
        .eq('cpf', parseInt(cleanCPF));
  ...
  const checkNumeroRegistroExists = async (numeroRegistro: number, excludeId?: number) => {
    try {
      let query = supabase
        .from('formulario_cepic')
        .select('id')
      ...
      console.error('Erro ao verificar número de registro:', error);
      return false;
    }
  };
  ...
  const handleAddUser = async () => {
    try {
      // Validações básicas
      if (!formData.nome || !formData.email || !formData.cpf || !formData.celular) {
        toast({
          title: "Erro",
      ...
      // Verificar número de registro se foi informado
      let numeroRegistro = parseInt(formData.numero_registro) || await getNextNumeroSorteio();
      
      if (formData.numero_registro) {
        const numeroExists = await checkNumeroRegistroExists(numeroRegistro);
        if (numeroExists) {
          toast({
            title: "Erro",
            description: "Este número de registro já existe no sistema.",
            variant: "destructive",
          });
          return;
        }
  ...
  const handleEditUser = async () => {
    ...
    try {
      // Validações básicas
      if (!formData.nome || !formData.email || !formData.cpf || !formData.celular || !formData.numero_registro) {
        toast({
          title: "Erro",
      ...
      const cpfExists = await checkCPFExists(formData.cpf, selectedUser.id);
      if (cpfExists) {
        toast({
          title: "Erro",
          description: "Este CPF já está cadastrado em outro usuário.",
          variant: "destructive",
        });
        return;
      }

      // Verificar se número de registro já existe (excluindo o usuário atual)
      const numeroRegistro = parseInt(formData.numero_registro);
      const numeroExists = await checkNumeroRegistroExists(numeroRegistro, selectedUser.id);
      if (numeroExists) {
        toast({
          title: "Erro",
          description: "Este número de registro já existe em outro usuário.",
          variant: "destructive",
        });
        return;
      }
