...
const Success = () => {
  ...
  const numeroSorteio = location.state?.numeroSorteio;
  const {
    data: numeroRegistro
  } = useQuery({
    queryKey: ['ultimo-numero-registro'],
    queryFn: async () => {
      const {
        data,
        error
      } = await supabase.from('formulario_cepic').select('numero_registro').order('created_at', {
        ascending: false
      }).limit(1).maybeSingle();
      if (error) throw error;
      return data?.numero_registro ?? null;
    }
  });
